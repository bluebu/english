...
**分类色**在根目录 `palette.css`，是全站唯一真源，和打卡单共用同一套五色：

| 打卡单（作业动作） | 色 | 打卡评价（内容分类） |
|---|---|---|
| 听 · 指读 | 蓝 `--c-listen` | **G3**（Wonders） |
| 读 · 点读 | 橙 `--c-read` | **超8** |
| 词 · 抄写 | 绿 `--c-word` | **单词** |
| 语法 | 青 `--c-gram` | **语法** |
| 练 · AI | 紫 `--c-drill` | （备用，新分类先用它） |

前四行是强对应；G3 那行是约定——Wonders 在打卡单上归 `[读]`（橙），但橙已经给了超8，
就用剩下区分度最好的蓝。

⚠️ **分类色里没有红**：红专属「分数越差越红」。语法原本跟打卡单用红，
2026-08-25 换成青——否则「红色的分类」会和「红色的差分数」在同一页上打架。
`homework/specs/` 里两条手写死的 `[#d97070] 故事习题` 也一并改成了 `[练]`，
现在全站没有任何硬编码色，一律走 `palette.css`。

怎么接的：
- 目录页 / 当日小结 `link` 了 `palette.css`，分类色由生成脚本以行内 `--c` / `--cbg` 注入
  （`CAT_COLOR` 在两个脚本里各一份，fallback 值和 CSS 里一致，CSS 没加载也不会掉色）
