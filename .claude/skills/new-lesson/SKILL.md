...
- **句子骨架 = 主语区 ▏谓语区(所有关通用 · 招牌 · 必用)**:句子拆两大区,以 `+` 为界;每区色块**下方一条横线 + 区名**标出范围(对标 Reed-Kellogg subject|predicate)。横线/区名用 `.zone` 的 `::before/::after` 浮层画,**不挤占排版** → 砖块照旧平排、同基线对齐。标准结构:
  ```html
  <div class="sentence sb">
    <span class="zone z-who"><span class="brick who" data-say="名词作主语"><span class="role">名词</span><span class="word">Tom</span></span></span>
    <span class="glue">+</span>
    <span class="zone z-pred">
      <span class="brick pred-v"><span class="role">&nbsp;</span><span class="word">likes</span></span>
      <span class="brick what" data-say="名词作宾语"><span class="role">名词</span><span class="word">cats</span></span>
    </span>
  </div>
  ```
  - `.zone.z-who` = 主语区(蓝横线 +「主语」);`.zone.z-pred` = 谓语区(橙横线、**刻意弱化** +「谓语」),托住"**谓语整块**"。
  - **谓语整块 = 主语之外的一切;里面放什么随关递进**:L1 只动作词 → L2 +宾语(绿) → L4 +系动词(紫)·表语(玫红)。这正是"先 主▏谓,再往谓语里拆"的可视化。
  - **动作词槽 `.brick.pred-v`**:橙砖块、`.role` 留 `&nbsp;` 只占位 → **不显示也不朗读「动词」**(动词词类到 L4 才登场);它不加 `data-say`、不可点。
  - **题目(`.task`)里区名自动隐藏**(横线保留)以免泄题——无需手动处理,CSS 已按 `.task .zone::after` 关掉。
  - 只有主语+单动词(无宾语,如 `Dogs run`)时,谓语区里就只有动作词槽,横线照样托住它。
...
- 🔊 **点击朗读 = 词类 + 角色**:
  - **进了句子的核心词**(占主语/谓语/宾语/系动词/表语槽)念 **「词类作成分」**:`data-say="名词作主语"`、`"名词作宾语"`、`"代词作主语"`、`"代词作宾语"`、`"动词作谓语"`、`"形容词作表语"`…(颜色仍按成分 class)。
  - **独立词汇**(封面 chips、四家族、动作动词列举)与**形态展示**(时态 walked/walk、比较级 taller/tallest)念**纯词类**:`data-say="名词"`/`"动词"`/`"形容词"`。
  - **修饰/连接词**(形容词/副词当定状、介词、连词、than)念**纯词类**。
  - **特例**:be 念 `"系动词"`;第 4 关表语若是形容词(形容词尚未教)念 `"表语"`;第 1 关只念成分(`主语`/`谓语`)。
...
- **一个词念词性还是成分?** 它的**词性一旦在前面关教过**就念词性,没教过就念成分。具体到**谓语区里的动作词**:L1(只教主▏谓、不拆)整块谓语念「谓语」;**L2–L3 动作词槽 `.brick.pred-v` 静默不念**(动词词类未教,且谓语身份已由区域横线表达);L4 起动词正式登场,念「动词作谓语」(be 念「系动词」、表语念「表语」)。
...
- 口诀:**看颜色 + 听声音,知道它是什么词、在当什么角色。**

## 四、注意事项(踩过的坑)
1. **"谓语"是大谓语**(主语之外的整块 = complete predicate),不是只指动词。`eat an apple` 整块都是谓语;`eat`=动词、`an apple`=宾语属于"谓语内部",留到后面关再拆——**讲到时点一句即可,别展开成长段**。**可视化一律用「二」的"主语区 ▏谓语区"骨架**:谓语区那条横线托住整块谓语,宾语/系动词+表语等就嵌在区内,一眼看出它们在谓语*里面*、不与谓语平级。**切忌**把单独动词块标成「谓语」再让宾语与它平级(那与本条自相矛盾,是第2关 review 踩过的坑)。
