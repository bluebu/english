# /grammar review — 关卡内容 · 权威对标审稿

> `/grammar` 的子命令。给一关(或一段新增内容)做"语言学 + 教学法"审稿:**语法对不对、术语合不合年级、序列顺不顺、例句与题目稳不稳**——逐条对照权威给 ✓ / ⚠️ / ✗ + 依据 + 建议。

先读根 `CLAUDE.md`(体系准则)与目标 `grammar/lesson-NN.html`(及本次 `git diff`,聚焦新增部分)。
...
5. **题目设计**:区分度、干扰项有没有考到要害、是否"靠颜色 / 位置泄题"、是否覆盖该关核心概念(别只考边角)。
